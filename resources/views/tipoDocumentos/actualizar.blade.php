<div class="modal fade" id="actualizar<?=$num?>" data-backdrop="static" data-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
    <div class="modal-dialog">
        ...
        <div class="modal-header">
            <h5 class="modal-title" id="staticBackdropLabel">ACTUALIZAR TIPO DE DOCUMENTO</h5>
        </div>
        <div class="modal-body">
            <form action="{{ route('documento.update',$documento->id) }}" method="POST">
            @method('PUT')
            @csrf
            <input class="form-control" {{ old('nombre') }} value="{{ $documento->nombre }}" type="text" name="nombre">
            <hr>
